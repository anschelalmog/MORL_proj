...
class ActionTrackingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        step_data = {
            ...
            'iso_sell_price': info.get('iso_sell_price', iso_info.get('sell_price', iso_info.get('sell_prices', [0.0])[
                -1] if iso_info.get('sell_prices') else 0.0)),
            'iso_buy_price': info.get('iso_buy_price', iso_info.get('buy_price', iso_info.get('buy_prices', [0.0])[
                -1] if iso_info.get('buy_prices') else 0.0)),
            'dispatch_cost': info.get('dispatch_cost', iso_info.get('dispatch_cost',
                                                                    iso_info.get('dispatch_costs', [0.0])[
                                                                        -1] if iso_info.get(
                                                                        'dispatch_costs') else 0.0)),
            'reserve_cost': info.get('reserve_cost', iso_info.get('reserve_cost', iso_info.get('reserve_costs', [0.0])[
                -1] if iso_info.get('reserve_costs') else 0.0)),
            'shortfall': info.get('shortfall', iso_info.get('shortfall',
                                                            iso_info.get('shortfalls', [0.0])[-1] if iso_info.get(
                                                                'shortfalls') else 0.0)),
    ...
    def plot_episode_results(self, episode_num: int, save_path: str):
        ...
        bar_dispatch = ax5.bar([bar_positions[0]], [total_dispatch], width=width, color='lightblue',
                               label='Dispatch Cost')
        ...
        if total_pcs >= 0:
            bar_pcs = ax5.bar([bar_positions[2]], [total_pcs], width=width, color='lightgreen',
                              label='PCS Exchange Cost')
        else:
            bar_pcs = ax5.bar([bar_positions[2]], [total_pcs], width=width, color='darkgreen',
                              label='PCS Exchange Revenue')
        ...
        if episode_data and any('pcs_actions' in d for d in episode_data):
            ...
            if pcs_actions and isinstance(pcs_actions[0], list) and len(pcs_actions[0]) > 0:
                for agent_idx in range(len(pcs_actions[0])):
                    agent_actions = [step_actions[agent_idx] if isinstance(step_actions, list) and len(
                        step_actions) > agent_idx else 0.0
