...
class TestMOSACPolicyDetailed:
    ...
    def test_action_sampling_and_log_prob(self, setup_environment):
        ...
        # Apply squashing and calculate log prob correction
        actions = th.tanh(raw_actions)
        log_prob = normal_dist.log_prob(raw_actions)

        # Apply squashing correction: log_prob -= sum(log(1 - tanh(actions)^2))
