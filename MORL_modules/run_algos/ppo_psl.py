...
from create_energy_net_env import create_energy_net_env
import gymnasium
class CollectorTask:
...
def run(args):
    ...
    if args.env_name == "energy_net":
        args.env_name = create_energy_net_env()
    ...
    print('start:',
          evaluate(args, target_policy.state_dict(), {'ob_rms': envs.venv.ob_rms, 'obj_rms': envs.venv.obj_rms}))
    ...
    rollouts = RolloutStorage(num_steps=args.num_steps, num_processes=args.num_processes,
                              ...
                              recurrent_hidden_state_size=target_policy.recurrent_hidden_state_size,
                              obj_num=args.obj_num)
    # start collect data
    torch.manual_seed(args.seed)
    iteration = 0
    ...
    while True:
        ...
        while cnt_done_workers < max_process_num:
            ...
            if pref_id < process_num:
                ...
                ideal_point = rl_results['ideal_point'] if ideal_point is None else np.maximum(ideal_point, rl_results[
                    'ideal_point'])
            cnt_done_workers += 1
