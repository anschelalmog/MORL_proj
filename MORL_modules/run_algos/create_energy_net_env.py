...
class GymnasiumToGymWrapperPSL(gym_old.Env):
    """
    ...
    def _fix_action_shape(self, action):
        ...
        Remove extra batch dimension from tensor if it has shape (1, n) -> (n,)
        """
    ...
    def _fix_observation_shape(self, observation):
        ...
        if isinstance(observation, (list, tuple)):
            ...
            if observation.dim() > 1 and observation.shape[0] == 1:
                observation = observation.squeeze(0)  # Remove first dimension if it's 1
    ...
    def step(self, action):
        ...
        if len(result) == 5:
            ...
            #for working with ppo  and other morl implementations
            info['obj'] = np.array(reward, np.float32)
...
def create_energy_net_env_gym_for_psl(**kwargs):
